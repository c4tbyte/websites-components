...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .card img {
    ...
    aspect-ratio: 1 / 1;
    object-fit: cover;
    border-radius: var(--lr-image-radius);
    background: #000;
    display: block;
  }
...
class LatestReleases extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (!this._resizeListenerAttached) {
      ...
      window.addEventListener("resize", () => {
        this._page = 0;
        this.shadowRoot.host.style.setProperty("--lr-columns", String(this.columns));
        if (this._allReleases.length > 0) this._renderPage();
      });
    }
  }
  ...
  _render() {
    ...
    root.host.style.setProperty("--lr-columns", String(this.columns));
  ...
  _renderPage() {
    ...
    const totalPages = Math.ceil(this._allReleases.length / perPage);
    arrowsWrap.hidden = !(totalPages > 1);

    const start = this._page * perPage;
    const pageItems = this._allReleases.slice(start, start + perPage);
  ...
  _goToPage(page) {
    const totalPages = Math.ceil(this._allReleases.length / this.perPage);
